...
@Component({
	...
	template: `
		<div class="relative flex w-full items-center group-data-vertical:w-auto group-data-vertical:flex-col">
			<div brnSliderTrack class="bg-muted rounded-none data-horizontal:h-1 data-horizontal:w-full data-vertical:h-full data-vertical:w-1 relative grow overflow-hidden">
				<div
			...
			@for (i of _slider.thumbIndexes(); track i) {
				<span
					class="border-ring ring-ring/50 size-3 rounded-none border bg-white transition-[color,box-shadow] hover:ring-1 focus-visible:ring-1 focus-visible:outline-hidden active:ring-1 absolute block shrink-0 select-none after:absolute after:-inset-2"
					brnSliderThumb
				></span>
			}
		</div>
